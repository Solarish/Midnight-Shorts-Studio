...
function PathField({ label, value, onChange, compact = false, filter }: { label: string; value: string; onChange: (value: string) => void; compact?: boolean; filter?: string }) {
  ...
  return <div className={`path-field ${compact ? "compact" : ""}`}><label>{label}<span><input aria-label={label} value={value} onChange={(event) => onChange(event.target.value)} placeholder="ยังไม่ได้เลือกไฟล์"/><button type="button" onClick={() => setOpen(true)}>Open Finder…</button></span>{value && !compact && <small className="path-selection">Selected: {fileName}</small>}</label><RemoteFilePickerModal isOpen={open} onClose={() => setOpen(false)} onSelect={(selected) => { onChange(selected); setOpen(false); }} initialPath={directoryForPath(value)} mode="file" filter={filter} title={`เลือก ${label} จาก NAS`}/></div>;
}
