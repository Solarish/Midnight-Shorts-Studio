...
Updated: 2026-08-24 16:41:50 +07
...
- Checkpoint/resume and staged `--to` execution work; unit tests pass.
...
- The AE adapter now polls for a result file instead of assuming `DoScriptFile` is synchronous.
...
## Prepared fallback if AE file access remains unreliable

1. Generate a self-contained JSX runner that embeds the job payload instead of reading `ae-job.json`.
2. Keep result polling and add an AE-side milestone log so the CLI can identify whether failure occurs during template open, text binding, footage replacement, save, or close.
3. If AppleScript `DoScriptFile` remains unstable, test the AE application executable with `-r` in the dedicated empty session before considering any broader host change.
